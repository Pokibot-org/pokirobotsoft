...
static void cmp_ctrl_step(control_fifo_t *ctrl_fifo)
{
    ...
    int16_t new_val_l =
        CONTROL_FF_L * (float)ctrl_fifo->set_speed.sl + CONTROL_P_L * (float)err_l + CONTROL_I_L * (float)ctrl_fifo->err_fsum_l;
    int16_t new_val_r =
        CONTROL_FF_R * (float)ctrl_fifo->set_speed.sr + CONTROL_P_R * (float)err_r + CONTROL_I_R * (float)ctrl_fifo->err_fsum_r;
